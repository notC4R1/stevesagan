...
client.on("messageCreate", async (message) => {
    if (message.author.username != "LFG_BOT") {
        ...
        if (message_string[0] == "!LFG") {
            ...
            //Bot is triggered with '!LFG' and will split the string into 3 parts.
            if (isNaN(message_string[1])) {
                message.channel.send(
                    "Please use an integer after !LFG to set number of players"
                );
            } else if (typeof message_string[2] == "undefined") {
                message.channel.send(
                    "Please enter the name of the game you wish to play"
                );
            } else if ((message_string[1] > 4) | (message_string[1] < 0)) {
                message.channel.send("Request a number between 1 through 4");
            } else {
                const sentMessage = await message.channel.send(
                    `${author} wants to play ${message_string[2]} with ${message_string[1]} people!`
                );
                sentMessage.react("😩");
                ...
                sentMessage
                    .awaitReactions({
                        ...
                        max: message_string[1],
                    })
                    .then(() => {
                        gameName = sentMessages.get(sentMessage.id);
                        message.channel.send(
                            `The party is ready to play ${gameName}!`
                        );
                    });
            }
        }
...
client.on("messageCreate", async (message) => {
    const collectorFilter = (message) =>
        message.content.includes("dota", "Dota");
